...
const CLUB_LOGOS = {
  ...
  "Charlton":            "https://upload.wikimedia.org/wikipedia/en/f/f5/Charlton_Athletic_FC_crest.svg",
  ...
  "Huddersfield":        "https://upload.wikimedia.org/wikipedia/en/4/43/Huddersfield_Town_AFC_crest.svg",
  ...
  "Bradford":            "https://upload.wikimedia.org/wikipedia/en/0/04/Bradford_City_AFC_crest.svg",
  "Swindon":             "https://upload.wikimedia.org/wikipedia/en/a/a3/Swindon_Town_FC.svg",
  "Oldham":              "https://upload.wikimedia.org/wikipedia/en/a/a2/Oldham_Athletic_AFC_%28emblem%29.svg",
...
function checkQualifier(player, season, qualifier) {
  ...
  switch (qualifier.type) {
    ...
    case 'last_name_matches_nationality': {
      ...
      const lastInitial = normName(parts[lastNameStart])[0];
      const natInitial  = normName((player.nationality || '').trim())[0];
      return !!lastInitial && !!natInitial && lastInitial === natInitial;
    }
    default:
      return true;
  }
...
function getRejectionReason(player, rowConfig) {
  ...
  for (const q of quals) {
    ...
    if (q.type === 'last_name_matches_nationality') {
      ...
      let lastIdx = parts.length - 1;
      for (let i = 1; i < parts.length - 1; i++) { if (ARTICLES.has(parts[i].toLowerCase())) { lastIdx = i; break; } }
      const lastInitial = normName(parts[lastIdx])[0].toUpperCase();
      const natInitial  = normName((player.nationality||'').trim())[0].toUpperCase();
      if (lastInitial !== natInitial)
        return `${player.name}'s last name starts with ${lastInitial} but their nationality (${player.nationality}) starts with ${natInitial} — they must match.`;
    }
...
function populateDateSelect() {
  ...
  for (let d = totalDays; d >= 0; d--) {
    ...
    const label = `${months[parseInt(mm, 10) - 1]} ${parseInt(dd, 10)} (#${puzzleNum})`;
